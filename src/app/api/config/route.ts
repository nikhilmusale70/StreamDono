...
const configSchema = z.object({
  ...
  overlayVolume: z.number().int().min(0).max(100).optional(),
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const updateData: Record<string, unknown> = {
      ...
      overlayVolume: data.overlayVolume ?? existing?.overlayVolume ?? 80,
    ...
    const config = await prisma.streamerConfig.upsert({
      ...
      create: {
        ...
        overlayVolume: updateData.overlayVolume as number,
